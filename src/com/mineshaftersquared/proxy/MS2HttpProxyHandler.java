...
public class MS2HttpProxyHandler implements MS2Proxy.Handler {
    ...
    public boolean onGet(String url, Map<String, String> headers, InputStream in, OutputStream out, MS2Proxy ms2Proxy) {
        ...
        if (skinMatcher.matches()) {
            ...
            if (data == null) {
                data = new SimpleHTTPRequest(ms2Proxy.routes.getSkinURL(username)).doGet();
            }
            ...
            if (data == null) {
                data = new SimpleHTTPRequest(ms2Proxy.routes.getCloakURL(username)).doGet();
            }
            ...
            String response = this.authMultiplayerAction("checkserver", request[1], ms2Proxy);

            this.sendResponse(out, "text/plain", response.getBytes(Charset.forName("utf-8")));

            return true;
        } else if (joinserverMatcher.matches()) {
            UniversalLauncher.log.info("JOINSERVER");
            String[] request = url.split("[?]");
            String response = this.authMultiplayerAction("joinserver", request[1], ms2Proxy);
    ...
    private String authMultiplayerAction(String action, String data, MS2Proxy ms2Proxy) {
        UniversalLauncher.log.info("Proxy - auth - action: " + action + ", data - " + data);
        ...
        request.addGet(data);
        return new String(request.doGet(Proxy.NO_PROXY), Charset.forName("utf-8"));
    }
